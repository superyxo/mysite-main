...
(function ($) {
	...
	$.fn.cleanHtml = function () {
		var html = $(this).html();
		// alert(html.replace(/(<br>|\s|<p><br><\/p>|&nbsp;)*$/g, ''));
	...
	$.fn.wysiwyg = function (userOptions) {
		var editor = this,
				...
				$.each(files, function (idx, fileInfo) {
					if (/^image\//.test(fileInfo.type)) {
						$.when(readFileIntoDataUrl(fileInfo)).done(function (dataUrl) {
							...
							giveImageName(dataUrl, fileInfo.name);
						}).fail(function (e) {
			...
			giveImageName = function(dataUrl, name){
				$(editor).find('img').each(function(i, obj){
					if( $(obj).attr('src') === dataUrl ){
						$(obj).attr('name', name);
					}
				});
			},
			...
			bindToolbar = function (toolbar, options) {
				...
				toolbar.delegate('input[type=file][data-' + options.commandRole + ']', 'change', function () {
					restoreSelection();
	...
	$.fn.wysiwyg.defaults = {
		...
		commandRole: 'edit',
		activeToolbarClass: 'btn-inverse',
